...
When someone wants to audit or improve an existing pricing *page* (not the pricing strategy itself), run a **teardown** that scores it across two axes and returns prioritized fixes:
...
**Fast check — the "paste test":** paste the pricing URL into an LLM and ask "what are the plans and prices?" If it can't answer correctly, an AI shopping for your buyer can't either — that failure is the finding.
